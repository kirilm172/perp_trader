...
async def test_collect_feed_skips_high_slippage():
    config = BotConfig(max_slippage_pct=5.0, position=PositionConfig(usd_amount=100, leverage=1))
    feed_queue = asyncio.Queue()
    data_feed = DataFeed(exchanges=[], markets=[], feed_queue=feed_queue, config=config)
...
async def test_collect_feed_accepts_low_slippage():
    config = BotConfig(max_slippage_pct=10.0, position=PositionConfig(usd_amount=100, leverage=1))
    feed_queue = asyncio.Queue()
    data_feed = DataFeed(exchanges=[], markets=[], feed_queue=feed_queue, config=config)
